...
`paths.CMake` が作成されたら、CMakeの設定手順に進みます。

ビルドフォルダを作成します。通常、 ` ./build/msvc10-amd64`, ` ./build/make-x86`, `./build/netbeans` など `./build/<ビルド専用フォルダ>` の二段階に分けてビルド手順を作成します。しかし、話を簡潔にするために、以下のスニペットでは `./build` を使用します。
...
すべての必要なパスが適切に構成されている場合、このようなものが表示されるはずである。
...
その後、オプションで以下を実行することができる。

``` bash
cmake-gui .
```

これにより、 CMake 用の GUI クライアントが起動され、 CMake 設定変数を**微調整**できます。たとえば、**プリコンパイルヘッダ**を ON または OFF にしたり、 C++ **RTTI** 設定を変更したり、 make ベースのビルドの設定を **Debug** から **Release** に、またはその逆に変更したりすることができます。

また、コマンドラインで  `cmake` の手順を実行しなくても、 `CMake-gui` で CMake 全体の設定手順を行うこともできます。
